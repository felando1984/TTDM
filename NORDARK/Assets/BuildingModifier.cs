namespace Mapbox.Unity.MeshGeneration.Modifiers
{
	...
	[CreateAssetMenu(menuName = "Mapbox/Modifiers/BuildingModifier")]
	public class BuildingsModifier : GameObjectModifier
	{
		public override void Run(VectorEntity ve, UnityTile tile)
		{
			var min = ve.MeshFilter.sharedMesh.subMeshCount;
			var mats = new Material[min];
			Debug.Log("ve" + ve.Transform.name + ":" + min);
			...
			for (int i = 0; i < min; i++)
			{
				Material mat = new Material(Shader.Find("HDRP/Lit")); // "Diffuse"
				try
				{
					mat.color = new Color((float)(rnd.NextDouble()), (float)(rnd.NextDouble()), (float)(rnd.NextDouble()), 1f);//probabilityToColor(indicators[busServiceAvailability.GetCurrentStep()]);
				}
